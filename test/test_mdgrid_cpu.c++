...
const suite<"mdgrid_cpu"> _ = [] {
    ...
    "mds() and staging_mds() point to same data"_test = [] {
        constexpr auto elem_desc = tyvi::mdgrid_element_descriptor<int>{ .rank = 2, .dim = 3 };
        using mdg                = tyvi::mdgrid<elem_desc, std::dextents<std::size_t, 3>>;

        auto grid    = mdg(3, 4, 5);
        auto mds_A   = grid.mds();
        auto smds_A  = grid.staging_mds();
        ...
        // Read through staging_mds — should see same data
        for (const auto idx : tyvi::sstd::index_space(smds_A)) {
            for (const auto Midx : tyvi::sstd::index_space(smds_A[idx])) {
                expect(smds_A[idx][Midx] == 42);
            }
        }
    };

    "span() and staging_span() return same pointer"_test = [] {
        constexpr auto elem_desc = tyvi::mdgrid_element_descriptor<int>{ .rank = 1, .dim = 3 };
        using mdg                = tyvi::mdgrid<elem_desc, std::dextents<std::size_t, 3>>;

        auto grid = mdg(3, 4, 5);
        expect(grid.span().data() == grid.staging_span().data());
    };
            ...
            for (const auto Midx : tyvi::sstd::index_space(staging_mds_B[idx])) {
                ...
                if (ok1 and ok2) {
                    ...
                    expect(staging_mds_B[idx][Midx] == 7)
                        << "expected " << 7 << ", got " << staging_mds_B[idx][Midx];
                }
            }
        }
    };
